...
2. 作る前に `artifact-design` skill (bundled) を読み、設計方針を決めてから書く
   ...
   - **初見の読者向けに書く**
     - 専門用語前提で数値表を並べた report は書き直しになった。
     - 冒頭に前提用語 3-5 個を平易に定義する (例: OKLCH、コントラスト比、dE)
     - 数値を出す前に「何をしたか / どう測ったか / なぜその測り方か」を説明する
     - 発見は「試した -> 結果 -> なぜそうなるか」の物語順で書き、
       結論の一覧表は先頭に置く
     - 実データで見せる: 色見本 (chip)、CSS で描く図解 (帯図等)、before/after 比較表
   - **実装物にデモ / 検証ページが含まれる場合**、そのページ自体にも最初から
     説明を組み込む: 「このページの使い方」パネル (開くと何が走るか、
     表示される数字の意味、操作すると何が起きるか) と、
     各セクション直下の「この表の読み方 / 何を見れば何が分かるか」の短文
...
5. plan と同じ commit、または直後の 1 commit で追加する
   - **作り終えたらユーザーに聞かずその場で commit する** (2026-08-19 決定。
     report は実装が終わった時点の記録物であり、都度の commit 可否確認は
     無駄なやり取りになるため)。通常の実装成果物 (「依頼されたときだけ
     commit する」) とは別枠の扱いである

この節は plan の**内容**についてのルールなので、リポジトリ固有の commit 規約
(署名の要否、message の言語等) はそのリポジトリの `AGENTS.md` / `CLAUDE.md` に従う。
push は別判断とし、リポジトリの push 規約 (承認要否等) に従う。
...
- **background 実行では必ず `< /dev/null` を付ける**。stdin が閉じないと codex が
  `Reading additional input from stdin...` のまま無期限に固まる (2026-08-16 実測。
  CPU がほぼ 0 のまま進まなければこれを疑い、kill して stdin を閉じて再実行する)
